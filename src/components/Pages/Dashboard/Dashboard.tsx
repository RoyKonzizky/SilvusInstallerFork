...
export function Dashboard() {
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className={`grid grid-cols-${isOddLength ? '3' : '2'} gap-4`}>
                {elements.map((element, index) => (
                    <div key={index} className={`bg-gray-900 text-xs p-3 rounded-xl ${isOddLength && index === elements.length - 1 ? 'col-span-2' : ''}`}>
                        {element}
                    </div>
                ))}
            </div>
